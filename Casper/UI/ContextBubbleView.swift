...
/// Casper logo view — uses the character image, falls back to emoji.
private struct PepperLogo: View {
    var size: CGFloat = 32

    var body: some View {
        if let image = NSImage(named: "casper-character") ?? Bundle.main.image(forResource: "casper-character") {
            Image(nsImage: image)
                .resizable()
...
/// The Casper Context Bubble — a branded floating panel that shows
