...
final class SettingsDictationTestController: ObservableObject {
    ...
    func stop() {
        ...
        Task { @MainActor in
            ...
            self.lastError = text == nil ? "Casper could not transcribe that sample." : nil
...
struct SettingsView: View {
            ...
            if !hasAccessibilityPermission || !hasInputMonitoringPermission {
                SettingsCard("Permissions") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Text("Both permissions are required for hotkeys and pasting to work reliably. If Casper doesn't appear in Input Monitoring, click + and select it from Applications.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 24) {
            SettingsCard("Cleanup") {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("When enabled, Casper runs local cleanup with the selected cleanup model from the Models section.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            SettingsCard("Cleanup prompt") {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Casper uses this prompt before adding OCR context and correction hints.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
            ...
            SettingsCard("Context") {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("Casper uses high-quality OCR on the frontmost window and adds the result to the cleanup prompt. When learning is enabled, Casper does a high-quality OCR check about 15 seconds after paste and only keeps narrow, high-confidence corrections.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private var modelsSection: some View {
        VStack(alignment: .leading, spacing: 24) {
            SettingsCard("Speech model") {
                ...
                Text("Casper uses this model for speech recognition everywhere in the app.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    private var recognizedVoicesSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            Text("Casper auto-creates reusable voice prints from speaker-tagged lab reruns. Marking more than one voice print as \"This is me\" is allowed.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            if transcriptionLabController.filteredEntries.isEmpty {
                if transcriptionLabController.searchText.isEmpty {
                    ContentUnavailableView(
                        ...
                        description: Text("Make a few dictations in Casper and they will appear here.")
                    )
                    .frame(maxWidth: .infinity, minHeight: 280)
                } else {
                    ContentUnavailableView(
                ...
                VStack(alignment: .leading, spacing: 18) {
                    if appState.trelloToken.isEmpty {
                        ...
                        if !appState.trelloBoards.isEmpty {
                            ...
                            Text("You can also say a board or list name when speaking — Casper will match it automatically. \(appState.trelloBoards.count) boards, \(appState.trelloBoards.flatMap(\.lists).count) lists loaded.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        } else {
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            SettingsCard("Meeting Transcription") {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Text("When enabled, Casper can detect video calls and offer to transcribe them locally. Requires Screen Recording permission for system audio capture.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
...
private struct TranscriptionLabSpeakerProfileEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if profile.recognizedVoiceID == nil {
                Text("This speaker only has a recording-local label because Casper could not build a reusable voice print from this sample.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
...
private struct ScreenRecordingRecoveryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Casper needs Screen Recording access. Grant it in System Settings, then return to Casper.")
                .font(.caption)
                .foregroundStyle(.red)
